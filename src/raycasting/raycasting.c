...
/*   Updated: 2026/05/18 16:24:36 by jose-ara         ###   ########.fr       */
/*                                                                            */
/* ************************************************************************** */
 ...
 * @return Void
 */
static inline void	invert_texture_x(t_const_vals const_vals,
		t_raycast *raycast)
{
	...
	{
		if (raycast->ray_angle > const_vals.rad_90_deg
			&& raycast->ray_angle < const_vals.rad_270_deg)
			raycast->texture_x_hp = 1.0f - raycast->texture_x_hp;
	}
}
...
void	draw_rays(t_game *game_wrap, t_player *player_info)
{
	...
	raycast.ray_angle = player_info->ang - (ONE_DEGREE
			* game_wrap->const_values.half_fov);
	...
	while (raycast.ray_ct < game_wrap->pixels_cols)
	{
		...
		if (player_info->p_moves)
			draw_line_simple(game_wrap, (float []){game_wrap->map_view->width
				/ 2, game_wrap->map_view->height / 2},
				(float []){((raycast.ray_x / CUBSIZE) * game_wrap->tile_size)
				- game_wrap->offset_x, (raycast.ray_y / CUBSIZE)
				* game_wrap->tile_size - game_wrap->offset_y}, RAY_COLOR);
...
void	draw_sprites(t_game *game_wrap, t_player *player_info)
{
	...
	{
		obj_is_alive(game_wrap, player_info, &obj_render, i);
		i++;
	}
	if (player_info->p_moves)
