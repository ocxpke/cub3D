...
/*   Updated: 2026/05/18 16:24:17 by jose-ara         ###   ########.fr       */
/*                                                                            */
/* ************************************************************************** */
		...
		t_object_render *obj_render, int index)
{
	...
	obj_render->dist_ratio = (game_wrap->obj_ordered[index].dist_to_player
			* CUBSIZE) / game_wrap->const_values.max_player_view_dist;
		...
		t_player *player_info, t_object_render *obj_render, int index)
{
	...
	obj_render->delta_x = game_wrap->obj_ordered[index].x_pos
		- player_info->posx;
	obj_render->delta_y = game_wrap->obj_ordered[index].y_pos
		- player_info->posy;
		...
		&& (obj_render->delta_y > -0.2f && obj_render->delta_y < 0.2f))
	{
		...
		posy = (int)game_wrap->obj_ordered[index].y_pos;
		posx = (int)game_wrap->obj_ordered[index].x_pos;
		game_wrap->map[posy][posx] = '0';
		return (0);
	}
